...
public abstract class CachedObject<T> where T : UnityEngine.Object
{
    ...
    public T I(Transform root)
    {
        ...
        if (cache_ == null)
        {
            Debug.LogWarning(string.Format("transform {0} not found",Path));
        }
...
public class CachedComponent<T> where T : Component
{
    ...
    T Getter(Transform root, Func<Transform, T> func)
    {
        ...
        if (component_ == null)
        {
            DebugPro.Log(root.name + "/{0}".Formats(finder_.Path) + " don't have component " + typeof(T).Name, DebugPro.Tag.Global);
        }
...
{
    T behaviour_;
    public T NullCheck(Animator root)
    {
        if (behaviour_ == null)
        {
            DebugPro.Log(root.name + " dont have component " + typeof(T).Name, DebugPro.Tag.Global);
        }
